    ...
    <>
      <nav className="flex sticky top-0 p-6 tracking-widest z-10 bg-[#004840] border-b-2 border-gray-400 ">
          ...
          >
            ...
            >
              ...
              {logged && dropdown === "customer" && (
                <ul className="text-white flex flex-col w-[180px] font-bold absolute top-7 left-1  bg-black">
                 {
                  customers.map(customer=> <li key={customer?.id} className="text-white border-b-2 p-3 hover:bg-red-700 border-gray-400">
                    <NavLink to={customer.to} className=""> {customer.link}</NavLink>
                  </li>)
                 }
                </ul>
              )}
            ...
            >
              ...
              {logged && dropdown === "provider" && (
                <ul className="text-white flex flex-col w-[180px]   font-bold absolute top-7 left-1  bg-black">
                  {
                  providers.map(provider=> <li key={provider?.id} className="text-white border-b-2 p-3 hover:bg-red-700 border-gray-400">
                    <NavLink to={provider.to} className=""> {provider.link}</NavLink>
                  </li>)
                 }
                </ul>
              )}
        ...
        </div>
        {
          ...
          className=" relative"
          >
            <img src={img2} alt="" className="rounded-full border h-[30px] w-[30px]" />
            {
              dropdown==="profile"&& 
